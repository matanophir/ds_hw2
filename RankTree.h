...
public:
    ...
    template <typename U>
    shared_ptr<T> remove_node(U&& dataT);  
...
template <typename T, typename C_e, typename C_lt>
class RankTree<T,C_e,C_lt>::Node
{
public:
    Node(shared_ptr<T> data_ptr,RankTree<T,C_e,C_lt>* tree ): data_ptr(data_ptr), tree(tree),height(0), extra(0){};
    ~Node() = default;
    ...
    void update_height();
    int get_bs_factor();
    friend class RankTree<T,C_e,C_lt>;
...
template <typename T, typename C_e, typename C_lt>
int RankTree<T,C_e,C_lt>::Node::get_height(shared_ptr<Node> node) // add const  & maybe TODO
{
    if (node == nullptr)
    {
        return -1;
...
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::Node::update_height()
{
    height = 1 + std::max(get_height(left), get_height(right));
}

template <typename T, typename C_e, typename C_lt>
int RankTree<T,C_e,C_lt>::Node::get_bs_factor()
{
    return get_height(left) - get_height(right);
}

template <typename T, typename C_e, typename C_lt>
bool RankTree<T,C_e,C_lt>::Node::operator==(T &other_data)
{
...
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::rr_roll(shared_ptr<Node> B)
{
    ...
    //updating extra field
    int al_extra_path;
    if (al!=nullptr)
        al_extra_path= al->extra + A->extra + B->extra;
    A->extra = A->extra + B->extra;
    B->extra = B->extra - A->extra; // A->extra is the updated one ofc 👆 
    if (al!=nullptr)
        al->extra = al_extra_path -(B->extra + A->extra);

    return A;
}

template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::ll_roll(shared_ptr<Node> B)
{
    ...
    int ar_extra_path;
    if (ar!=nullptr)
        ar_extra_path= ar->extra + A->extra + B->extra;
    A->extra = A->extra + B->extra;
    B->extra = B->extra - A->extra; // A->extra is the updated one ofc 👆 
    if (ar!=nullptr)
        ar->extra = ar_extra_path -(B->extra + A->extra);
...
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::lr_roll(shared_ptr<Node> C)
{
...
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::rl_roll(shared_ptr<Node> C)
{
 ...
 */
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::balance(shared_ptr<Node> curr_node)
{
    int old_height;
    int bs_factor;
    while (curr_node)
 ...
 *
 * @param curr
 * @return shared_ptr<RankTree<T,C_e,C_lt>::Node>
 */
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::_choose_roll(shared_ptr<Node> curr)
{
 ...
 */
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::_create_node(shared_ptr<T> data_ptr)
{
 ...
 */
template <typename T, typename C_e, typename C_lt>
shared_ptr<typename RankTree<T,C_e,C_lt>::Node> RankTree<T,C_e,C_lt>::_delete_node(shared_ptr<Node> to_del)
{
 ...
 */
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::_update_parent_child(shared_ptr<Node> parent,
                                   shared_ptr<Node> new_child,
                                   shared_ptr<Node> old_child)
{
    if (parent == nullptr)
 ...
 */
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::_swap_nodes(shared_ptr<Node> to_del, shared_ptr<Node> to_swap)
{
 ...
 */
template <typename T, typename C_e, typename C_lt>
StatusType RankTree<T,C_e,C_lt>::add_node(shared_ptr<T> data_ptr)
{
    if (root == nullptr)
    {
    ...
    new_node->extra = -extra_path; // to cancel out the adds done before the creation..
...
template <typename T, typename C_e, typename C_lt>
template<typename U>
shared_ptr<T> RankTree<T,C_e,C_lt>::remove_node(U&& dataT) // can refactor with get
{
...
template <typename T, typename C_e, typename C_lt>
template <typename U>
shared_ptr<T> RankTree<T,C_e,C_lt>::get(U&& dataT)
{
    shared_ptr<Node> curr_node = root;
...
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::in_order(shared_ptr<Node> node)
{
    if (node == nullptr)
    {
        return;
...
template <typename T, typename C_e, typename C_lt>
void RankTree<T,C_e,C_lt>::in_order_insertArr(int* ids)
{
...
template <typename T, typename C_e, typename C_lt>
template<typename Func>
void RankTree<T,C_e,C_lt>::in_order_Wfunc(Func func)
{
    shared_ptr<Node> curr = root;
    shared_ptr<Node> prev = nullptr;
 ...
 *
 * @param dataT the obj to check with
 */
template <typename T, typename C_e, typename C_lt>
template<typename U>
void RankTree<T,C_e,C_lt>::add_extra(U&& dataT, int to_add)
{
    ...
    while (curr_node != nullptr)
    {
        ...
        {
            if (right_streak)
                curr_node->extra-=to_add;
            if (curr_node->left != nullptr)
                curr_node->left->extra+= to_add;
            return;
        }
        if (*curr_node < dataT)
        {
...
template <typename T, typename C_e, typename C_lt>
template <typename U>
int RankTree<T,C_e,C_lt>::get_extra(U&& dataT)
{
    shared_ptr<Node> curr_node = root;
    int extra=0;
...
// template <typename T, typename C_e, typename C_lt>
// void RankTree<T,C_e,C_lt>::_update_smallest_elem()
// {
//     shared_ptr<Node> curr = root;
//     if (curr == nullptr) {
